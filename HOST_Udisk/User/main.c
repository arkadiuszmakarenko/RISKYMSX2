...
int main (void) {
    ...
    /* Dump RCC reset-cause flags. RMVF bit must be set to clear, which we
     * do at the end of this block so subsequent reads see only new causes.
     *   bit 26 PINRSTF  = NRST pin reset
     *   bit 27 PORRSTF  = power-on / power-down reset
     *   bit 28 SFTRSTF  = software reset (NVIC_SystemReset)
     *   bit 29 IWDGRSTF = independent watchdog reset
     *   bit 30 WWDGRSTF = window watchdog reset
     *   bit 31 LPWRRSTF = low-power / standby reset
     */
    printf ("[boot %u] rst=0x%08x\r\n",
        (unsigned)boot_count, (unsigned)RCC->RSTSCKR);
    if (g_fault_mepc != 0 || g_fault_mcause != 0) {
        printf ("[fault] mepc=%08x mcause=%08x mtval=%08x sp=%08x ra=%08x\r\n",
            (unsigned)g_fault_mepc, (unsigned)g_fault_mcause,
            (unsigned)g_fault_mtval, (unsigned)g_fault_sp, (unsigned)g_fault_ra);
        g_fault_mepc = 0;
        g_fault_mcause = 0;
    }
    printf ("SystemClk:%d\r\n", SystemClock);
