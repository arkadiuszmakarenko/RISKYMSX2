...
void SystemInit (void) {
    ...
    /* Board uses an external 25 MHz active oscillator on HSE — set BYPASS
     * before HSEON is written by SetSysClock(). HSEBYP is bit 18 of RCC->CTLR;
     * the clears above already left HSEON off, so this is the safe window. */
