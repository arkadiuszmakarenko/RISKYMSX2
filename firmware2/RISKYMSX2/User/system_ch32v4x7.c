...
/* RISKYMSX2 board uses a 25 MHz HSE BYPASS oscillator. The WCH header
 * defaults HSE_VALUE to 8 MHz, which makes RCC_GetClocksFreq() return
 * 3.125x too low on every HSE-PLL profile (USART/TIM/I2C/SPI baud rates
 * silently wrong). Override before including ch32v4x7.h - the header
 * has a #ifndef HSE_VALUE guard.
 *
 * NOTE: currently unused - the active profile is SYSCLK_*_HSI which
 * uses HSI_VALUE, not HSE_VALUE. Kept here so flipping back to a HSE
 * profile in the future doesn't need to remember the override. */
...
/* Boot is on HSI @400 MHz (HSI * 20 / 1 / 2). Why not HSE?
 * On this board (HSE = 25 MHz active oscillator, BYPASS mode), starting
 * the USBHS PLL from HSE@25 MHz brownouts the on-chip LDO during the
 * 25 -> 480 MHz PLL startup transient -- confirmed by an infinite boot
 * loop where the last print before each reset is `u4` (just after
 * `RCC->CTLR |= RCC_USBHSPLLON`). WCH's CFGR0[25:24] USBHSPLLSRC and
 * CFGR0[29:28] REFCLK source-select bits are ignored on this silicon;
 * the USBHSPLL reference is hard-locked to HSE at power-on reset.
 *
 * Workaround: never run USBHSPLL while system clock is on HSE. Boot on
 * HSI (PLL source = HSI @20 MHz, SYSCLK = 400 MHz) and bring up USBHS
 * from there. The system stays on HSI permanently; HSE is unused.
 *
 * Cost: HSI @20 MHz vs HSE @25 MHz is slightly less accurate. For this
 * firmware's USART1 921600 baud that's well within tolerance -- the
 * WCH header's HSI_VALUE = 20 MHz matches the chip. */
