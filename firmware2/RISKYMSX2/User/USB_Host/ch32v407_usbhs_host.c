...
void USBHS_RCC_Init (FunctionalState sta) {
    if (sta) {
        if ((RCC->CTLR & RCC_USBHSPLLRDY) == 0) {
            ...
            /* CFGR2[29:28] REFCLK and CFGR2[27:26] USBHSPLLSRC are
             * read-only on this silicon (POR-locked to HSE @ 25 MHz).
             * The WCH library's RCC_USBHSPLLCLKConfig / ReferConfig
             * writes are silently ignored here and were the root cause
             * of USBHS enumeration failures on the SYSCLK_*_HSE PLL
             * profiles. Just turn the PLL on. */
            RCC->CTLR |= RCC_USBHSPLLON;
