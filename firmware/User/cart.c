 ...
 *         zero-wait-state SRAM, plus one hand-scheduled asm handler
 *         (Cart_EXTI0_KonamiNOSCC_Handler) that serves KONAMINOSCC as a
 *         direct VTF entry without the dispatcher hop. Reads hit PSRAM
 ...
 *   page 3 (0xC000..0xFFFF) : mirror of page 2 (or RAM, depending)
 *
...
static void Run8kASCII (void) {
    ...
    if ((ctrl & CART_RD_MASK) == 0U) {
        /* READ. ASCII 8k read slot index from the legacy v303 firmware
         * (also matches the actual MSX spec for ASCII 8k):
         *   0x6000..0x7FFF -> read slot 1 (bankOffsets[1])
         *   0x8000..0x9FFF -> read slot 2 (bankOffsets[2])
         *   0xA000..0xBFFF -> read slot 3 (bankOffsets[3])
         *   0x4000..0x5FFF -> not cart (BIOS / fixed header)
         *
         * The write formula below uses slot 0..3 which correspond to
         * write addresses 0x6000/0x6800/0x7000/0x7800 (slot 0 is the
         * "6000h" write target and is not used by reads - writes to
         * 0x6000 set bankOffsets[0] which is never read). */
        uint32_t rslot;
        if      (address >= 0xA000U) rslot = 3U;
        else if (address >= 0x8000U) rslot = 2U;
        else if (address >= 0x6000U) rslot = 1U;
        else { GPIOB->CFGHR = CART_BUS_OFF;
               EXTI->INTFR  = EXTI_INTENR_MR0;
               while ((GPIOE->INDR & CART_SLTSL_MASK) == 0U) { }
               return; }
