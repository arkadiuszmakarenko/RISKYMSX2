 ...
 * Description        : USB host port / enumeration shared types for the
 *                      RISKYMSX2 firmware.  Based on the WCH
 *                      v303Firmware/User/USB_Host/usb_host_config.h,
 *                      ported to the CH32V407 USBHS controller (this
 *                      firmware's ch32v4x7 silicon has only USBHS -
 *                      no USBFS SIE).
 *
 *                      The structure layouts and DEF_* identifiers match
 *                      the v303 source so the migrated usb_disk.c can use
 *                      the same ROOT_HUB_DEVICE / HostCtl bookkeeping.
 *********************************************************************************/
...
/* The v407 has two USBHS controllers (USBHS1 + USBHS2). We use USBHS1 -
 * the driver brings it up via USBHS_Host_Init(ENABLE). */
#define DEF_USB_PORT                  0x01
...
#define DEF_BUS_RESET_TIMEOUT         30     /* USBHS bus reset timeout, ms */
#define DEF_RE_ATTACH_TIMEOUT         100    /* ms to wait for device after reset */
#define DEF_WAIT_USB_TOUT_200US       1000
#define DEF_CTRL_TRANS_TIMEOVER_CNT   60000  /* control-transfer poll count */
...
/* RootHUB / HUB structures (lifted verbatim from v303 usb_host_config.h).
...
/* USB Host Control Structure (interface bookkeeping) */
struct __HOST_CTL
{
...
/* Public enumerator state (declared in usb_disk.c, used by enumeration glue). */
